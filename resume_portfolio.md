...
| 제목                           | 요약                                                                                                                                                                 |
|------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Nest.js → Spring Boot 마이그레이션 | - 개발팀 내 기술 스택 통일화를 위해 Nest.js → Java/Spring Boot 마이그레이션 담당<br/>- 기존 API의 any 타입 요청/응답을 실제 데이터 수집 후 DTO 기반 계층 분리로 재설계<br/>- 40건 이상의 API를 Spring Boot 타입 시스템 기반으로 전환 |
| API 성능 최적화                   | - N+1 쿼리 해결 및 인덱스 최적화로 API 응답속도 40% 개선 (200ms → 120ms)<br/>- Swagger 응답 명세 작성 및 Custom 어노테이션 활용 컨트롤러 코드 간소화                                                        |
| 인프라 고가용성 확보                  | - AWS 기반 서버 이중화 및 ALB 트래픽 분산 구성<br/>- 사내 기획 고도화에 따른 추가 서비스 기능 개발 및 데이터 모델링 수행                                                                                      |
...
| 제목                  | 요약                                                                                                                  |
|---------------------|---------------------------------------------------------------------------------------------------------------------|
| 거래소 API 연동 및 백엔드 개발 | - 기존 Nest.js 시스템을 Spring Boot로 전환<br/>- 주요 거래소(업비트, 빗썸, 바이낸스 등) API 연동, 거래 내역 자동 수집<br/>- 거래 데이터 정제 및 손익 계산 대시보드 구현 |
...
| 제목           | 요약                                                                                                             |
|--------------|----------------------------------------------------------------------------------------------------------------|
| 인프라 및 백엔드 개발 | - 네이버 클라우드 플랫폼(NCP) 인프라 초기 세팅 및 배포 환경 구성<br/>- JavaScript/React 기반 프론트엔드 추가 기능 구현<br/>- 테이블 정의서 작성 및 데이터베이스 설계 |
...
| 대규모 데이터 마이그레이션    | - 에코인벤트(ecoinvent) 환경영향평가 원본 데이터 76만 건 정제 및 DB 정규화<br/>- 자사 시스템 스키마에 맞춘 데이터 마이그레이션 수행                                                                |
...
| 제목               | 요약                                                                                                        |
|------------------|-----------------------------------------------------------------------------------------------------------|
| 시스템 설계 및 프로젝트 관리 | - 탄소 배출량 데이터 모델링 및 확장 가능한 RESTful API 설계<br/>- 프로젝트 WBS 작성 및 백엔드/프론트엔드 개발 일정 관리                           |
| 성능 최적화           | - N+1 쿼리 문제 진단·해결 및 Redis 기반 캐싱으로 API 응답시간 55% 단축<br/>- Nginx HTTP/2 프로토콜 전환 및 Gzip 압축 적용으로 네트워크 전송 효율 개선 |
| 보안               | - Next.js RCE 취약점 긴급 패치 및 QueryDSL 의존성 버전 교체<br/>- 백엔드/프론트엔드 보안 취약점 점검 및 의존성 업데이트 수행                      |
